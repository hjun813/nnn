...
export function TaskChecklist({ jobId, tasks }: { jobId: string; tasks: Task[] }) {
  ...
  async function add(event: FormEvent<HTMLFormElement>) {
    ...
    setPendingId("new");
    setError("");
    const response = await fetch(`/api/jobs/${jobId}/tasks`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ type: data.get("type"), title: data.get("title"), status: "TODO", isRequired: true, sortOrder: localTasks.length }) });
    if (response.ok) {
      const data = await response.json();
      setLocalTasks((current) => [...current, data.task]);
      form.reset();
    } else setError("작업을 추가하지 못했습니다.");
    setPendingId(undefined);
    router.refresh();
  }
